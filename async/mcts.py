...
class MCTS:
    ...
    async def getMostVisitedAction(self, state, sim_count, verbose = False):
        info = await self.getActionInfo(state, sim_count)
        if verbose:
            for a in info:
    ...
    async def getActionInfo(self, state, sim_count):
        uid = state.getRepresentativeString()
        info = await self._getinfo(state, uid)
    ...
    async def _simulation(self, state):
        # terminal state
        ...
        v = -await self._simulation(child_state)
    ...
    async def _getinfo(self, state, uid):
        if not uid in self.info_map:
            action = state.getAction() # (action, key)
            raw_policy, value = await self.nn.predict(state.getNnInput())
